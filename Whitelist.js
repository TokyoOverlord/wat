...
local api_key = "4dfdace9-2734-42e5-9e24-0c5dfb94d0ae"
...
local serverRequest = http_request({
    Url = 'https://nutauth.ml/api/check?key='..api_key..'&license='..Key
}).Body
local response = HttpService:JSONDecode(serverRequest)
local serverResponse = response.data
...
local randomData = randomData:sub(1, -2)
if (hash(Key .. randomData) == serverResponse and rawequal(hash(Key .. randomData), serverResponse) and hash(Key .. randomData):len() == 64 and serverResponse:len() == 64) then
    isWhitelisted = true;
else
...
print("Cloud X")
loadstring(game:HttpGet"https://raw.githubusercontent.com/TokyoOverlord/wat/main/what.lua")()
